...
{
    public enum JobType { Haul, Build, Farm, Craft, Cultivate }
    ...
    public sealed class Job
    {
        ...
        public readonly ResourceContainer haulSource;
        public readonly ResourceContainer haulDestination;
        public readonly ResourceType haulResource;
        public readonly int haulAmount;
        ...
        public Job(int id, ResourceContainer source, ResourceContainer destination, ResourceType resource, int amount)
        {
            this.id = id;
            type = JobType.Haul;
            haulSource = source;
    ...
    public sealed class JobBoard : MonoBehaviour
    {
        ...
        public Job AddHaul(ResourceContainer source, ResourceContainer destination, ResourceType resource, int amount)
        {
            ...
            if (!destination.ReserveDropoff(resource, amount))
            {
                source.ReleasePickup(resource, amount);
                return null;
            }

            var job = new Job(_nextId++, source, destination, resource, amount);
            _jobs.Add(job);
            return job;
        }
        ...
        public Job ReserveBest(Vector3 workerPosition, Func<JobType, float> preference)
        {
            ...
            foreach (Job job in _jobs)
            {
                ...
                float priority = preference(job.type);
                if (priority <= 0f) continue;

                float distance = Vector3.Distance(workerPosition, job.ReservationTarget);
                if (job.IsHaul && job.haulSource != null && job.haulDestination != null)
                    distance += Vector3.Distance(job.haulSource.transform.position, job.haulDestination.transform.position) * 0.2f;

                float score = priority * 100f - distance;
        ...
        public void MarkPickedUp(Job job)
        {
            if (job != null && job.IsHaul) job.PickupCompleted = true;
        }
        ...
        void ReleaseHaulReservations(Job job)
        {
            if (job == null || !job.IsHaul) return;
            if (!job.PickupCompleted && job.haulSource != null)
                job.haulSource.ReleasePickup(job.haulResource, job.haulAmount);
            if (job.haulDestination != null)
                job.haulDestination.ReleaseDropoff(job.haulResource, job.haulAmount);
        }
    }
}
